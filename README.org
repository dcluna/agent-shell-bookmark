...
The =agent-shell-ol= module provides an org-link type for agent-shell sessions.  Store a link to a running session and follow it later to resume the exact conversation.

** Installation

Add to your config alongside =agent-shell-bookmark=:
...
In any =agent-shell= buffer, store a link with:

- =C-c l= (=org-store-link=)

Then in an org buffer, insert it with:
...
1. If a buffer with the stored session ID is still open, switch to it.
2. If no buffer exists but the session ID is known, resume the session with =agent-shell-start=.
3. If no session ID was stored (deferred session), offer to spawn a new agent in the stored project directory.
